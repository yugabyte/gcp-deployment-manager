...
readonly FILE_TO_UPDATE="template/instance.jinja"
...
# Current Version in template/instance.jinja
current_version="$(grep -A 1 "yb-version" "${FILE_TO_UPDATE}" | grep "value" | cut -d '"' -f 2)"
...
# Update Version
sed -i -E "/key: \"yb-version\"/,+1 s/[0-9]+.[0-9]+.[0-9]+.[0-9]+/"${release_version}"/" "${FILE_TO_UPDATE}"
